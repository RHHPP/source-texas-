...
if tonumber(data.sender.user_id) == tonumber(Texas) then
return false
end
end
...
Redis:srem(Texas..'BaN:In:Tuasl',Info_User)  
return send(msg.chat_id,msg.id,Reply_Status(Info_User,'➢ تم الغاء حظره من الصانع ').Reply,"md",true)  
end 
end
end
if text == "➢ الغاء الامر" then
Redis:del(Texas..msg.sender.user_id.."bottoken")
Redis:del(Texas..msg.sender.user_id.."botuser")
Redis:del(Texas..msg.sender.user_id.."make:bot")
...
Redis:set(Texas..msg.sender.user_id.."run:bot",true)
send(msg.chat_id,msg.id,"➢ ارسل معرف البوت ليتم تشغيله")
return false 
end
---ايقاف البوتات
if text and Redis:get(Texas..msg.sender.user_id.."stop:bot") then
...
Redis:set(Texas..msg.sender.user_id.."stop:bot",true)
send(msg.chat_id,msg.id,"➢ ارسل معرف البوت ليتم ايقافه")
return false 
end
--الاشتراك الاجباري 
if Redis:get(Texas.."ch:addd"..msg.sender.user_id) == "on" then
Redis:set(Texas.."ch:addd"..msg.sender.user_id,"off")
...
Redis:del(Texas.."chfalse")
Redis:set(Texas.."chfalse",ch)
Redis:del(Texas.."ch:admin")
Redis:set(Texas.."ch:admin",da.result.invite_link)
else
send(msg.chat_id,msg.id,'➢ المعرف خطأ او البوت ليس مشرف في القناه ',"md",true)  
end
end
if text == "➢ تفعيل الاشتراك الاجباري" then
Redis:set(Texas.."ch:addd"..msg.sender.user_id,"on")
...
if text and Redis:get(Texas..msg.sender.user_id.."make:bot") == "devuser" then
...
local bottoken = Redis:get(Texas..msg.sender.user_id.."bottoken")
local botuser = Redis:get(Texas..msg.sender.user_id.."botuser")
...
Redis:sadd(Texas.."userbots",botuser)
Redis:del(Texas..msg.sender.user_id.."bottoken")
Redis:del(Texas..msg.sender.user_id.."botuser")
Redis:del(Texas..msg.sender.user_id.."make:bot")
...
if text and Redis:get(Texas..msg.sender.user_id.."make:bot") == "token" then
...
if Redis:sismember(Texas.."userbots",botuser) then
send(msg.chat_id,msg.id, "\n➢ عذرا هذا البوت مصنوع بالفعل","md",true)  
return false 
end 
Redis:set(Texas..msg.sender.user_id.."botuser",botuser)
Redis:set(Texas..msg.sender.user_id.."bottoken",text)
Redis:set(Texas..msg.sender.user_id.."make:bot","devuser")
...
Redis:set(Texas..msg.sender.user_id.."make:bot","token")
send(msg.chat_id,msg.id, "\n➢ ارسل توكن البوت الان","md",true)  
return false 
end 
----------end making
----broadcast all
if Redis:get(Texas.."all:texting") then
if text == "الغاء" or text == '➢ الغاء الامر' then   
Redis:del(Texas.."all:texting")
send(msg.chat_id,msg.id, "\n➢ تم الغاء الاذاعه","md",true)  
return false 
end 
Redis:set(Texas.."3z:gp",text)
Redis:del(Texas.."all:texting")
...
for k,v in pairs(list) do
https.request("https://api.telegram.org/bot"..bottoken.."/sendmessage?chat_id="..v.."&text="..URL.escape(Redis:get(Texas.."3z:gp")).."&parse_mode=Markdown")
end
end
end
Redis:del(Texas.."3z:gp")
Redis:del(Texas.."all:texting")
send(msg.chat_id,msg.id,"➢ تم انتهاء الاذاعه في كل البوتات","html",true)
end
if Redis:get(Texas.."all:texting:pv") then
if text == "الغاء" or text == '➢ الغاء الامر' then   
Redis:del(Texas.."all:texting:pv")
send(msg.chat_id,msg.id, "\n➢ تم الغاء الاذاعه","md",true)  
return false 
end 
Redis:set(Texas.."eza3a:pv",text)
Redis:del(Texas.."all:texting:pv")
...
for k,v in pairs(list) do
https.request("https://api.telegram.org/bot"..bottoken.."/sendmessage?chat_id="..v.."&text="..URL.escape(Redis:get(Texas.."eza3a:pv")).."&parse_mode=Markdown")
end
end
end
Redis:del(Texas.."eza3a:pv")
Redis:del(Texas.."all:texting:pv")
send(msg.chat_id,msg.id,"➢ تم انتهاء الاذاعه في كل البوتات","html",true)
end
if text == "➢ اذاعه عام للمجموعات" then
Redis:set(Texas.."all:texting",true)
send(msg.chat_id,msg.id,"ارسل النص الان","html",true)
end
if text == "➢ اذاعه عام للمشتركين" then
Redis:set(Texas.."all:texting:pv",true)
send(msg.chat_id,msg.id,"ارسل النص الان","html",true)
end
-------screen -ls
...
if text == "➢ فحص" then
Redis:del(Texas.."All:pv:st")
Redis:del(Texas.."All:gp:st")
for folder in io.popen('ls'):lines() do
if folder:match('@[%a%d_]') then
m = Redis:get(folder)
x = {m:match("(.*)&(.*)$(.*)+(.*)")}
...
list = Redis:smembers(bot_id.."ChekBotAdd") 
lt = Redis:smembers(bot_id.."Num:User:Pv") 
Redis:incrby(Texas.."All:gp:st",tonumber(#list))
Redis:incrby(Texas.."All:pv:st",tonumber(#lt))
end
end
send(msg.chat_id,msg.id,'\n➢ احصائيات جميع البوتات المصنوعه \n ➢ عدد المجموعات '..Redis:get(Texas.."All:gp:st")..' مجموعه \n➢ عدد المشتركين '..Redis:get(Texas.."All:pv:st")..' مشترك',"html",true)
end
-----ban all
...
-------delete 
if text and Redis:get(Texas..msg.sender.user_id.."make:bot") == "del" then
if text == "الغاء" or text == '➢ الغاء الامر' then   
Redis:del(Texas..msg.sender.user_id.."make:bot")
...
Redis:srem(Texas.."userbots",text:gsub("@",""))
send(msg.chat_id,msg.id, "\n➢ تم حذف البوت بنجاح","md",true)  
return false 
end 
if text == "➢ حذف بوت" then
Redis:set(Texas..msg.sender.user_id.."make:bot","del")
...
devbot = x[3]
bottoken = x[4]
Redis:sadd(Texas..'3ddbots',botuser..' » '..devbot)
end
end
local list = Redis:smembers(Texas..'3ddbots')
if #list <= 100 then
...
----end--3dd
if text and Redis:get(Texas..msg.sender.user_id.."setchannel") then
if text == "الغاء" or text == '➢ الغاء الامر' then   
Redis:del(Texas..msg.sender.user_id.."setchannel")
send(msg.chat_id,msg.id, "\n➢ تم الغاء تعيين قناه السورس","md",true)  
return false 
end 
if text:match("@(.*)") then
local ch = text:match("@(.*)")
Redis:set(Texas.."chsource",ch)
...
if tonumber(Texas) ~= tonumber(FilesJson.BotId) then
return send(msg_chat_id,msg_id,'➢ عذرا هاذا الملف غير مطابق مع البوت يرجى جلب النسخه الحقيقيه')
end -- end botid
...
Y = Y + 1
Redis:sadd(Texas..'total',v)  
end
end
end
if text == "جلب نسخه" then
local UsersBot = Redis:smembers(Texas.."total")
local Get_Json = '{"BotId": '..Texas..','  
if #UsersBot ~= 0 then 
...
--brodcast
if Redis:get(Texas..msg.sender.user_id.."brodcast") then 
if text == "الغاء" or text == '➢ الغاء الامر' then   
Redis:del(Texas..msg.sender.user_id.."brodcast") 
send(msg.chat_id,msg.id, "\n➢ تم الغاء الاذاعه","md",true)  
return false 
end 
local list = Redis:smembers(Texas.."total") 
...
if text == "➢ اذاعه" then
Redis:set(Texas..msg.sender.user_id.."brodcast",true)
send(msg.chat_id,msg.id,"➢ ارسل الاذاعه الان")
return false 
end
---fwd
if Redis:get(Texas..msg.sender.user_id.."brodcast:fwd") then 
if text == "الغاء" or text == '➢ الغاء الامر' then   
Redis:del(Texas..msg.sender.user_id.."brodcast:fwd")
...
Redis:set(Texas..msg.sender.user_id.."brodcast:fwd",true)
...
chinfo = Redis:get(Texas.."ch:admin")
send(msg.chat_id,msg.id,'\n➢ عليك الاشتراك في قناة البوت لاستخذام الاوامر\n\n'..chinfo..'')
return false 
end
if not Redis:get(Texas.."twsl") then
if msg.sender.user_id ~= tonumber(Texas) then
if Redis:sismember(Texas..'BaN:In:Tuasl',msg.sender.user_id) then
return false 
end
if msg.id then
Redis:setex(Texas.."Twasl:UserId"..msg.date,172800,msg.sender.user_id)
...
if text and Redis:get(Texas.."free:bot") then
return send(msg.chat_id,msg.id,"➢ الوضع المجاني معطل من قبل مطور الصانع")
end
if text == "/start" then
if not Redis:sismember(Texas.."total",msg.sender.user_id) then
Redis:sadd(Texas.."total",msg.sender.user_id)
end
reply_markup = LuaTele.replyMarkup{type = 'keyboard',resize = true,is_personal = true,
data = {
{
...
---making user
if text and Redis:get(Texas..msg.sender.user_id.."make:bot") then
if text == "➢ الغاء" then
Redis:del(Texas..msg.sender.user_id.."make:bot")
...
Informationlua:close()
os.execute('cp -a ./source/. ./@'..botuser..' && cd @'..botuser..' && chmod +x * && screen -d -m -S '..botuser..' ./Run')
Redis:set(Texas..msg.sender.user_id.."my:bot",botuser)
Redis:sadd(Texas.."userbots",botuser)
Redis:del(Texas..msg.sender.user_id.."make:bot")
...
if text == "➢ صنع بوت" then
if Redis:get(Texas..msg.sender.user_id.."my:bot") then
return send(msg.chat_id,msg.id,"➢ عفوا لديك بوت بالفعل احذفه اولا")
end
Redis:set(Texas..msg.sender.user_id.."make:bot",true)
...
if Redis:get(Texas..msg.sender.user_id.."my:bot") then
local botuser = Redis:get(Texas..msg.sender.user_id.."my:bot")
os.execute("sudo rm -fr @"..botuser)
os.execute("screen -X -S "..botuser.." quit")
Redis:srem(Texas.."userbots",botuser)
Redis:del(Texas..msg.sender.user_id.."my:bot")
send(msg.chat_id,msg.id, "\n➢ تم حذف البوت بنجاح","md",true)  
else
...
if tonumber(data.message.sender.user_id) == tonumber(Texas) then
return false
end
Run(data.message,data.message)
elseif data and data.luatele and data.luatele == "updateMessageEdited" then
local Message_Edit = LuaTele.getMessage(data.chat_id, data.message_id)
if Message_Edit.sender.user_id == Texas then
return false
end
